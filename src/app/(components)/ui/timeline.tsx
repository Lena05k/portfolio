...
import { SparklesTwo } from './sparklesTwo';
                    ...
                    {data.map((item, index) => (
                        ...
                        >
                            <div className="sticky flex flex-col md:flex-row z-40 items-center top-40 self-start max-w-xs lg:max-w-sm md:w-full" >
                                <div className="h-10 absolute left-3 md:left-3 w-10 rounded-full bg-white dark:bg-black flex
                                items-center justify-center">
                                    <div className="h-4 w-4 rounded-full bg-neutral-200 dark:bg-neutral-800 border border-[#48b6ff] dark:border-neutral-700 p-2" />
                                </div>
                                <h3 className="hidden sm:text-left sm:mb-0 md:block md:pl-20 text-sm sm:text-md md:text-base lg:text-xl xl:text-2xl 2xl:text-3xl font-bold text-neutral-500 dark:text-neutral-500 ">
                                    {item.title}
                                </h3>
                            </div>

                            <div className="relative pl-16 md:pl-4 md:pr-4 lg:pl-20 w-full">
                                <div className="absolute inset-0 h-full w-full transform scale-[0.95] rounded-full blur-2xl"></div>
                                <div  className="relative flex flex-col gap-5 justify-end items-start shadow-lg shadow-[#48b6ff]/[0.3]  border border-[#48b6ff] dark:border-gray-800 px-4 py-8 h-full overflow-hidden rounded-2xl
                                    text-sm sm:text-md md:text-base lg:text-xl xl:text-2xl 2xl:text-3xl ">
                                    {item.name}
                                    {item.description}
                                    {item.steck}
                                    <Meteors number={20}/>
